...
export default function UserCard({user}){
    ...
    return(
        <button  className="py-3 px-4 flex gap-2 hover:bg-[#eff3f41a] transition-colors ">
            <img src={user.avatar} className= "w-10 h-10 rounded-full object-cover" alt="" />
                        <div className="flex-1 flex flex-col text-left ">
                            <div className="text-[15px] font-bold leading-5 text-[#e7e9ea] flex gap-1">
                                <Link to="/" className=" hover:underline "> {user.fullname} </Link>
                                {user?.verified && (
            ...
            <div >
                ...
                {following ? (
                     <Button  variant="white-outline"  size='small' onClick={()=> setFollowing(false)} className="group">
